...
//for weeding invlalid break & continue statement
void weed_bc_stmts(STMTS *stmts)
{
    ...
    case tree_stmt_kind_break:
        fprintf(stderr, "Error: invalid break statement now allowed (line %d)", stmts->stmt.lineno);
        exit(1);
    case tree_stmt_kind_continue:
        fprintf(stderr, "Error: invalid continue statement now allowed(line %d)", stmts->stmt.lineno);
        exit(1);
    case tree_stmt_kind_if:
        weed_bc_if(&(stmts->stmt));
    case tree_stmt_kind_switch:
        weed_bc_switch(&(stmts->stmt));
    }
    weed_bc_stmts(stmts->next);
}
//for weeding invlalid default case
// void weed_d_stmts(STMTS *stmts) {
...
{
    weed_bc_stmts(if_stmt->body);
    weed_bc_stmts(if_stmt->elsebody);
}

//checker for invalid break and continue in switch
void weed_bc_switch(SWITCH_STMT *switch_stmt)
{
    if (switch_stmt == NULL)
        return;
    weed_bc_cases(switch_stmt->cases);
}
...
void weed_bc_cases(CASES *cases)
{
    if (cases == NULL)
        return;
    if (cases->body->stmt.kind == tree_stmt_kind_continue)
    {
        fprintf(stderr, "Error: invalid continue statement now allowed(line %d)", cases->body->stmt.lineno);
        exit(1);
    }
